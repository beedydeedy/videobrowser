...
namespace DvrmsMetadataProvider {
    ...
    public class DvrmsMetadataProvider : BaseMetadataProvider {
        ...
        public override void Fetch() {
            ...
            while (attempt < 6 && !success) {
                try {
                    ...
                    Logger.ReportWarning("Dvrms Metadata Provider: Fetching metadata failed on attempt " + attempt + " for " + (Item as Video).VideoFiles.First());
                    attempt++;
                }
            }
        ...
        {
            Logger.ReportInfo("Dvrms Metadata Provider: Getting Metadata for " + (Item as Video).VideoFiles.First());
            using (new MediaBrowser.Util.Profiler("Dvrms Metadata Provider: Metadata extraction"))
            using (DvrmsMetadataEditor editor = new DvrmsMetadataEditor((Item as Video).VideoFiles.First())) 
            {
        ...
        public override bool NeedsRefresh() {
            return IsDvrms && new FileInfo((Item as Video).VideoFiles.First()).LastWriteTime > updateDate;
        }
    }
}
